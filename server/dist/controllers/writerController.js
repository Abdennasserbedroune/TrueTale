...
exports.createWriterController = createWriterController;
const http_status_codes_1 = require("http-status-codes");
const mongoose_1 = __importDefault(require("mongoose"));
const models_1 = require("../models");
const writerValidation_1 = require("../validation/writerValidation");
...
function createWriterController(feedService) {
    const createBook = async (req, res) => {
        ...
        const parseResult = writerValidation_1.createBookSchema.safeParse(req.body);
        ...
        try {
            ...
            await session.withTransaction(async () => {
                ...
                if (createdBook.status === "published") {
                    await feedService.record("book_published", {
                        userId,
            ...
            res.status(http_status_codes_1.StatusCodes.CREATED).json({ book: serializeBook(createdBook) });
        }
        catch (error) {
    ...
    const updateBook = async (req, res) => {
        ...
        const parseResult = writerValidation_1.updateBookSchema.safeParse(req.body);
        if (!parseResult.success) {
            res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json(formatValidationError(parseResult.error));
            return;
        }
        ...
        try {
            ...
            await session.withTransaction(async () => {
                const book = await models_1.Book.findById(id).session(session);
                if (!book) {
                    errorResponse = {
            ...
            res.status(http_status_codes_1.StatusCodes.OK).json({ book: serializeBook(updatedBook) });
        }
        catch (error) {
    ...
    const getBooks = async (req, res) => {
        ...
        const parseResult = writerValidation_1.paginationQuerySchema.safeParse(req.query);
        if (!parseResult.success) {
            res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json(formatValidationError(parseResult.error));
            return;
        }
        ...
        try {
            const [books, total] = await Promise.all([
                models_1.Book.find({ writerId, status: "published" })
                    .sort({ publishedAt: -1, createdAt: -1 })
                    .skip((page - 1) * limit)
                    .limit(limit),
                models_1.Book.countDocuments({ writerId, status: "published" }),
            ]);
    ...
    const getDrafts = async (req, res) => {
        ...
        try {
            const [drafts, total] = await Promise.all([
                models_1.Draft.find({ writerId })
                    .sort({ updatedAt: -1 })
                    .skip((page - 1) * limit)
                    .limit(limit),
                models_1.Draft.countDocuments({ writerId }),
            ]);
            const totalPages = total === 0 ? 0 : Math.ceil(total / limit);
            res.status(http_status_codes_1.StatusCodes.OK).json({
    ...
    const createDraft = async (req, res) => {
        ...
        const parseResult = writerValidation_1.createDraftSchema.safeParse(req.body);
        if (!parseResult.success) {
            res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json(formatValidationError(parseResult.error));
            return;
        }
        const writerId = new Types.ObjectId(req.user.userId);
        try {
            const draft = await models_1.Draft.create({
                ...parseResult.data,
                writerId,
            });
            res.status(http_status_codes_1.StatusCodes.CREATED).json({ draft: serializeDraft(draft) });
        }
        catch (error) {
    ...
    const updateDraft = async (req, res) => {
        ...
        const parseResult = writerValidation_1.updateDraftSchema.safeParse(req.body);
        if (!parseResult.success) {
            res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json(formatValidationError(parseResult.error));
            return;
        }
        try {
            const draft = await models_1.Draft.findById(id);
            if (!draft) {
                sendErrorResponse(res, {
            ...
            await draft.save();
            res.status(http_status_codes_1.StatusCodes.OK).json({ draft: serializeDraft(draft) });
        }
        catch (error) {
    ...
    const getStories = async (req, res) => {
        ...
        try {
            const [stories, total] = await Promise.all([
                models_1.Story.find({ writerId })
                    .sort({ createdAt: -1 })
                    .skip((page - 1) * limit)
                    .limit(limit),
                models_1.Story.countDocuments({ writerId }),
            ]);
            const totalPages = total === 0 ? 0 : Math.ceil(total / limit);
            res.status(http_status_codes_1.StatusCodes.OK).json({
    ...
    const createStory = async (req, res) => {
        ...
        const parseResult = writerValidation_1.createStorySchema.safeParse(req.body);
        if (!parseResult.success) {
            res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json(formatValidationError(parseResult.error));
            return;
        }
        ...
        try {
            ...
            await session.withTransaction(async () => {
                const story = new models_1.Story({
                    ...parseResult.data,
                ...
                if (createdStory.published) {
                    await feedService.record("story_published", {
                        userId,
            ...
            res.status(http_status_codes_1.StatusCodes.CREATED).json({ story: serializeStory(createdStory) });
        }
        catch (error) {
    ...
    const deleteStory = async (req, res) => {
        ...
        try {
            const story = await models_1.Story.findById(id);
            if (!story) {
                sendErrorResponse(res, {
    ...
    const getProfile = async (req, res) => {
        ...
        const writerId = new Types.ObjectId(req.user.userId);
        try {
            const user = await models_1.User.findById(writerId);
            if (!user) {
                sendErrorResponse(res, {
            ...
            const [totalBooks, publishedBooks, totalDrafts, totalStories] = await Promise.all([
                models_1.Book.countDocuments({ writerId }),
                models_1.Book.countDocuments({ writerId, status: "published" }),
                models_1.Draft.countDocuments({ writerId }),
                models_1.Story.countDocuments({ writerId }),
            ]);
    ...
    const updateProfile = async (req, res) => {
        ...
        const parseResult = writerValidation_1.updateProfileSchema.safeParse(req.body);
        if (!parseResult.success) {
            res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json(formatValidationError(parseResult.error));
            return;
        }
        ...
        try {
            ...
            await user.save();
            res.status(http_status_codes_1.StatusCodes.OK).json({ profile: serializeProfile(user) });
        }
        catch (error) {
...
//# sourceMappingURL=writerController.js.map