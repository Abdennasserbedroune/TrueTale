...
const mockBooks = [
    {
        title: "Tidal Dreams",
        description: "A serialized climate fiction saga following three generations of ocean engineers rebuilding floating cities.",
    ...
    {
        title: "Echo Vault",
        description: "An interactive noir mystery where readers unlock clues hidden across multiple timelines.",
        category: "Mystery",
        price: 18.0,
        status: "published",
    ...
    },
    {
        ...
        description: "A multimedia poetry chapbook exploring digital rituals, intimacy, and ecological dreaming.",
        category: "Poetry",
        price: 11.0,
        status: "published",
    ...
    },
    {
        ...
        description: "Investigative feature on solarpunk co-ops prototyping grid-independent infrastructure.",
        category: "Journalism",
        price: 9.5,
        status: "published",
...
const mockDrafts = [
    {
        ...
        content: "A collaborative worldbuilding draft documenting floating seed libraries across Pacific communities. Every lantern held a seed story, ready for the next port. The communities have developed intricate systems for sharing knowledge across vast distances...",
    },
    {
        title: "Clockwork City Sketches",
        content: "Urban exploration meets mechanical mystery. The city's underground systems run on a complex network of gears and pulleys, each with its own story to tell. I've been documenting the maintenance crews who keep everything running...",
    },
    {
        title: "Digital Rituals",
        content: "Exploring the intersection of technology and spirituality in modern worship. How do we create sacred spaces in virtual environments? What does it mean to commune through screens? These questions guide my research into digital ceremonies...",
    },
];
const mockStories = [
    {
        title: "The Last Librarian",
        content: "In a world where books have been digitized and forgotten, one librarian maintains the last physical library. Every day, she curates stories that refuse to be compressed into binary code, preserving the tactile experience of paper and ink...",
        published: true,
    },
    {
        title: "Neon Gardens",
        content: "Bioluminescent plants grow in the abandoned subway tunnels beneath the city. A group of urban explorers discovers this hidden ecosystem and must decide whether to protect it or expose it to a world that might destroy it...",
        published: true,
    },
    {
        title: "Memory Market",
        content: "In a near-future where memories can be bought and sold, a memory broker discovers a set of memories that shouldn't exist. These fragments suggest a different history than the one everyone remembers, and knowing the truth could cost everything...",
        published: false,
    },
];
const mockReviews = [
    {
        rating: 5,
        reviewText: "Absolutely brilliant! The worldbuilding is incredible and the characters feel so real.",
    },
    {
        rating: 4,
        reviewText: "Great story with a unique premise. The pacing could be improved in the middle section.",
    },
    { rating: 5, reviewText: "Couldn't put it down! The author's writing style is captivating." },
    {
        rating: 3,
        reviewText: "Good concept but the execution felt rushed. Would love to see a more developed version.",
    },
...
async function seedDatabase() {
    try {
        ...
        await Promise.all([
            ...
            models_1.Draft.deleteMany({}),
            models_1.Story.deleteMany({}),
            models_1.Review.deleteMany({}),
            models_1.Follow.deleteMany({}),
            models_1.FeedActivity.deleteMany({}),
        ]);
        ...
        for (let i = 0; i < mockDrafts.length; i++) {
            ...
            const draft = new models_1.Draft(draftData);
            await draft.save();
        ...
        for (let i = 0; i < mockStories.length; i++) {
            ...
            const story = new models_1.Story(storyData);
            await story.save();
        ...
        for (let i = 0; i < readers.length; i++) {
            for (let j = 0; j < Math.min(3, writers.length); j++) {
                ...
                const follow = new models_1.Follow(followData);
                await follow.save();
        ...
        for (const book of createdBooks.filter((b) => b.status === "published")) {
            const activity = new models_1.FeedActivity({
